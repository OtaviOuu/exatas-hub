defmodule ExatasHubWeb.UserAuthTest do
  use ExatasHubWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias ExatasHub.Accounts
  alias ExatasHub.Accounts.Scope
  alias ExatasHubWeb.UserAuth

  import ExatasHub.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ExatasHubWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      ExatasHubWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        ExatasHubWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
